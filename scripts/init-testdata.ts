...
const SUBMISSIONS = [
  ...
  // Admin global submissions (testing outside contest context)
  {
    subId: 7,
    username: "admin",
    problemName: "addition",
    ...
    submissionTime: formatDateTime(new Date(startTime.getTime() - 30 * 60 * 1000)),
    gradingTime: formatDateTime(new Date(startTime.getTime() - 30 * 60 * 1000)),
    gradingCompleteTime: formatDateTime(new Date(startTime.getTime() - 29 * 60 * 1000)),
    score: [100, 100, 100, 100],
    verdicts: ["AC", "AC", "AC", "AC"],
    times: [5, 8, 10, 25],
    memories: [800, 800, 800, 1200],
    returnCodes: [0, 0, 0, 0],
    status: [2, 2, 2, 2],
    subtaskScores: [0, 36, 64],
    totalScore: 100,
  ...
  },
  {
    ...
    submissionTime: formatDateTime(new Date(startTime.getTime() - 15 * 60 * 1000)),
    gradingTime: formatDateTime(new Date(startTime.getTime() - 15 * 60 * 1000)),
    gradingCompleteTime: formatDateTime(new Date(startTime.getTime() - 14 * 60 * 1000)),
    score: Array(152).fill(0).map((_, i) => i < 30 ? 100 : 0), // First 30 pass
    verdicts: Array(152).fill("WA").map((_, i) => i < 30 ? "AC" : "WA"),
    times: Array(152).fill(15),
    memories: Array(152).fill(1500),
    returnCodes: Array(152).fill(1).map((_, i) => i < 30 ? 0 : 1),
    status: Array(152).fill(2),
    subtaskScores: [10, 0, 0], // Only got subtask 1
    totalScore: 10,
    maxTime: 15,
    maxMemory: 1500,
  },
